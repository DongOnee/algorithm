...
1. BOJ 1761
    - Algorithm : LCA + DP
    - Talk. : 백준 알고리즘 분류 LCA 에 있는 문제이다. 하나의 데이터가 추가되었는데 왜 이렇게 헤맸는지.. 다시 구조를 보면서 비슷한 문제가 나왔을 때 바로바로 풀어 버리자.

## 20.03.06.
1. BOJ 11438
    - Algorithm : LCA + DP
    - Talk. : BOJ 11437 문제를 풀고 다른사람의 풀이를 보는중 엄청나게 시간이 단축한 사람이 있었다. 그분의 코드를 살펴 보니 parent 정보를 DP 식으로 담아서 풀이를 한게 눈에 띄었다. 이전에 SWEA 1855 문제의 해법을 볼때도 이와 비슷하게 되어 있었다. 그 원리를 파악하려고 많은 시간을 보냈고 결국 깨닿음을 얻었다.
    - sol
        - `parent_of_[idx][j]` 의 의미
            - idx 노드의 2<sup>j</sup> 떨어진 부모노드
            - `p = parent_of_[idx][j]` 라고 했을떄 `parent_of_[idx][j+1] = parent_of_[p][j]` 을 만족한다.
        - 그림을 그려서 생각해보면 더 이해가 잘 될것이다.

## 20.03.03.
1. BOJ 11437
    - Algorithm : LCA
    - Talk. : 공통의 조상을 찾는 문제이다. a, b 의 공통 조상을 찾는다고 했을때 모든 노드의 depth와 parent 정보를 담고 있는 후 a, b 중 depth 가 더 깊은 것을 끌어 올린다. 그 후 동일 depth 에서 값이 같아질때까지 둘다 끌어올리면 같아진 값이 공동 조상이 되는 방법이다.

## 20.02.10.
1. BOJ 3665
    - Algorithm : 위상 정렬
    - Talk : 문제의 카테고리를 알고 풀기 시작했었다. 위상 정렬. 꼬리에 꼬리를 물고, 앞에 꼬리가 처리될때 뒤의 꼬리들에게 영향을 주고 그 꼬리들중 하나를 시작으로 다시 처리를 하는 방식이라고 느낌적인 느낌으로 알고 있다. 근데 처리에 대한 부분에서 막혀서 ... 결국 답을 보고 말았다.
    - sol
        1. 이전 등수에 대해서 저장하고 그대로 복사하여 새로운 등수를 저장한다.
        2. 등수 변화에 대해서 새로운 등수 배열에 ++, -- 를 이용해서 처리를 한다.
            - 순위가 오른건 -- ( 숫자가 낮을수록 순위가 높은거니까. ...), 순위가 떨어진건 ++
        3. pair 를 만들어 <새로운 등수, 팀 번호> 페어로 만들어 vector 에 저장한다.
        4. sort 함수를 이용해서 정렬을 한다.
        5. 출력을 위한 처리를 해본다.
            - vector 의 크기가 n 이 아니면 "?"
            - vector 내부에 같은 등수가 있으면 "IMPOSSIBLE"
            - 아니면 다 출력

## 20.02.07.
1. BOJ 1976
    - Algorithm : Union-Find
    - Talk : 문제에 몇 가지 함정이 있었다. 보기좋게 걸려들어버렸고 다른사람 코드를 참고하여 해결 할 수 있었다.
        1. 가중치가 없다. 문제의 첫번째 줄을 다시 보면 "도시가 N개 있고 임의의 두 도시 사이에 길이 있을 수도, 없을 수도 있다." 라고 되어 있다. 이 말은 100만번을 왔다 갔다 해도 목표에 도달하기만 하면 된다는 것이다.
        2. 순서가 크게 상관 없다. 문제를 다시 읽어보면 "도시들의 개수와 도시들 간의 연결 여부가 주어져 있고, 동혁이의 여행 계획에 속한 도시들이 순서대로 주어졌을 때(중복 가능) 가능한지 여부를 판별하는 프로그램을 작성하시오." 라고 되어 있다. **여행 계획에 속한 도시들이 순서대로 주어졌을 때** 라는 말이 가장 큰 혼돈을 주었다.
    - Sol : group 을 이용해서 연결되어 이동할 수 있는 도시들을 그룹화 하였다. Union 함수를 이용해서 연결된 두 도시를 그룹화 하였고 find 함수를 이용해서 작은 숫자로 그룹화 하였다.

## 19.11.7.
1. SWEA 2383
    - Algorithm : Simulation
    - Talk : priority queue 를 이용하여 풀어 보았다. 나름대로 머리써서 풀었는데 결과를 보니 실행시간이 엄청 길게 나왔다. 다른사람의 코드(2383_sol_?.cpp)를 찾아 보았고 역시나 너무 어렵게 풀었다는걸 깨달았다. 
    - solv 1
        1. 모든 사람이 2개의 개단을 이용하는 경우의 수를 DFS 를 이용하여 구하였다.
            - 문제에서 계단은 2개라고 나와 있어서 경우의 수를 구하는데 큰 시간이 소요되지 않을거라 판단 하였다.
        2. simulation() 함수를 통해서 각 경우의 수를 직접 동작해 본다.
            - priority queue 를 만들어 {시간, 이용계단} 을 저장하였다.
                - 정렬은 시간이 작을수록, 이용계단이 클수록 top 에 오도록 하였다.
                - 여기서, 이용계단이 *음수* 이면 "시간"에 계단에 도착, *양수* 이면 "시간"에 계단 이동 완료 라고 하였다.
                - 이렇게 하면 같은 시간에 **계단 도착**하고 **계단 이동 완료**하더라도 **계단 이동 완료**가 먼저 실행되기 때문에 이후에 **계단 도착**에 지장이 없다.
            - useCntExit 배열에 계단을 이용중인 사람수를 저장하였고 waitExit 배열을 이용하여 계단을 이용하기 위해 기다리고 있는 사람의 수를 저장하였다.
            - priority queue 의 맨위(시간적 우선, 계단 이동 완료 우선) 인 것 부터 처리하였다.
                - 계단에 도착한사람 (이용계단이 음수) 이면
                    - 계단을 이용할 수 있는지 검사 (usdCntExit 을 검사)
                        - 이용할 수 있으면 useCntExit 을 증가, {현재 시간+계단 이동시간, -이용계단(양수로)} 를 priority queue 에 push.
                        - 이용할 수 없으면 waitExit 을 증가,
                - 계단 이동을 완료한 사람 (이용 계단이 양수) 이면
                    - useCntExit 을 감소
                    - waitExit 이 있으면 waitExit 감소, {현재 시간 + 계단 이동 시간, 이용계단} 을 priority queue 에 push, useCntExit 증가.
        - 자세한건 코드(2383.cpp) 참조...
    - solv 2
        1. 
...
1. SWEA 2382
    - Algorithm : Simulation
    - Talk : 굉장히 시간이 많이 걸렸다. 한심하게... 더 풀어서 정리해보고 싶은 문제이다. 만약 삼성 역량 테스트를 본다면 이런 문제가 나올것 같다.
        1. 미생물 이동
            1. 한 군집 이동
            2. side?? -> 처리
            3. 이미 도착한 군집이 있냐?
...
1. SWEA 1949
    - Algorithm : math
    - Talk : [해법](https://zetawiki.com/wiki/SWEA_1859_%EB%B0%B1%EB%A7%8C_%EC%9E%A5%EC%9E%90_%ED%94%84%EB%A1%9C%EC%A0%9D%ED%8A%B8)을 보고 깜짝 놀랐다. 이렇게 쉽게 풀수 있구나 싶었다. 나는 sort 를 이용해서 가장 비쌀때를 체크 해서 그떄까지 사고 팔기를 계산 했는데 이 방법은 뒤에서 부터 계산을 해서 sort 를 할 필요가 없었다...

## 19.10.22.
1. SWEA 1949
    - Algorithm : DFS
    - Talk : 가장긴 등산로를 구하는 문제이다. 멘탈 바사삭.
    - Issue
        1. BFS 로 푸려고 하면 굉장히 복잡하다.
        2. ios::sync_~~ 를했는데 scanf 를 쓰고 있어서 계속 틀렸다.

## 19.10.18.
1. BOJ 17406
    - Algorithm : DFS Simulation
    - Talk : 함수단위로 나워서 생각하고 풀면 EZ

2. BOJ 17471
    - Algorithm : DFS 를 이용해서 조합을 구하는 문제이다.
    - Talk : 삼성 A형 시험 기출 문제라고한다. 역시 구현 문제가 자주 나오는것 같다. 맨처음에 구역을 나눌때 알고리즘을 적용해야 되는 효율성을 생각했는데 그럴 필요가 없었다.... 그냥 무식하게 조합을 나누고 구역이 가능한지 확인하고 두 구역으로 나눠지면 계산하는 방식으로 풀이가 진행된다.

3. BOJ 1647
    - Algorithm : Union - Find
    - Talk : 위 문제랑 유사하지만 이 문제는 Edge 가 주어졌다. Union - Find 알고리즘을 사용하면 쉽게 풀 수 있다. 가중치가 작은 Edge 부터 Union 하고 Edge 의 vertex 가 같은 부모 노드를 가지고 있으면 스킵한다. 그리고 답을 구할때는 연결된 Edge 중에 가장 가중치가 큰것을 빼주면 알아서 2 구역으로 나뉘고, edge들의 가중치의 합이 최소가 될수 있다.

3. BOJ 17472
    - Algorithm : Union - Find
    - Talk : 문제는 두가지가 있다.
        1. 모든 섬은 1로 구성되어 있다. 이제 이걸 각각 나눠야 된다.
        2. 크루스칼 알고리즘을 쓴다.
        3. 모든 섬들이 연결되는지 확인하고 값을 구한다.
    - 나중에 코드 다듬어 봐야겠다.
...
1. BOJ 3653
    ...
    - Talk : **트리**에 한번 놀라고 **팬윅 트리** 에 두번 놀랐다.
    - Issue
        1. fail/3653.cpp : 가장 간단한 방법으로 풀어보았다. 링크드 리스트를 이용해서 앞에서 부터 읽으면서 찾으면 지운다음 맨 위에 삽입하는 식으로 구현하였다. 당연히 **시간초과~**
        2. 3653.cpp : 결국 검색을 통해서 솔루션을 찾아 보았다. **segment Tree** 를 구현하여 문제를 풀었다. **update**, **query** 함수를 이용해서 노드의 변화에 따른 트리 업데이트와 원하는 구간을 찾을 수 있었다. 하지만 시간이 오래 걸리길레 다른 방법이 있을까 하고 채점 현황에서 찾아 보았다.
        3. 3653_2.cpp : **팬윅 트리** 라는것에 놀라버렸고 `idx += idx&(-idx)` 이거 오래전에 생각한거 까먹고 있었는데 다시 상기 시켜주었다. 두고두고 생각해서 팬윅트리 마스터 해버리자.
...
1. BOJ 1406
    ...
    - Talk : **링크드 리스트**를 구현하고 커서의 이동, 삽입, 제거를 코딩하는 문제이다.
    - Issue
        1. 1406.cpp : 인덱스 접근으로 풀어보았다. 구조체 안에 **pointer**를 이용해서 이전 노드와 이후 노드를 방문하는 방법이 있지만 동적 할당을 쓰기 귀찮기도 하고 머리좀 써보자는 의도로 인덱스 접근으로 해보았다. 이 방법을 이용해서 코딩 하였을때 굉장히 코딩이 하기 싫었다. 그래서 좀 많이 엄청 오래 걸렸다.
        2. 1406_2.cpp : 위 방법으로 문제를 푼 후 **list STL** 을 이용해서 푼 답안을보고 경악을 하였다. 역시 STL 짱
...
1. BOJ 9375
    - Algorithm : DataStruct
    - Talk : `map` 을 이용하여 각 의상 종류당 갯수를 저장한 다음 **모든 조합의수 -1** 을 해준다. **-1**을 하는 이유는 아무것도 안입은 경우가 없어야 되기 때문니다. **모든조합의 수** 에는 각 **의상 종류의 수 +1** 을 해줘야 되는데 그 의상 종류를 안입는 경우도 있기 때문이다.

1. BOJ 16137
    - Algorithm : BFS
    - Talk : 이 문제가 레전드 문제. 이해를 잘못해서 몇 시간을 날렸는지 모르겠다. 
        - '오작교는 이처럼 매우 불안정하므로, 견우는 안전을 위해 **두 번 연속으로 오작교를 건너지는 않기로 했다.**' 라는 조건이 있다.
        - 나는 이 조건을 ~~간은 오작교를 여러번 건너지 않는다.~~로 이해하는 멍청이다. 이 조건 덕분에 더욱 복잡한 경우를 무시할 수 있다. 
        - '한 번 지은 오작교는 1분 동안 유지할 수 있다.' 라는 조건이 있다.
        - 나는 한칸에 1분만 있을 수 있다고 생각했다. 그러니까 (x, y) 에 t 시간에 도착 했다고 하면 t+1 시간에는 다른곳으로 꼭 이동해야 된다. 라고 생각을 하는 멍청이다.
...
1. Kakao Blind Test Problem 4
    - Algorithm : 탐색
    - Talk : EZ. `queries` 에서 하나씩 추출하고 모든 `words` 와 비교하는식으로 풀면 정확성에서 통과 받을 수 있다. 하지만 효율성에서 통과는 못받는다. 효율성통과를 받기위해 생각을 해봤는데 머리가 쪼개질듯 해서 해설을보니 **Trie자료구조** 라는 문자열 탐색에 최적화된 자료구조가 있다고 한다. 좀 나중에 공부해보자.

## 19.10.3.
1. Kakao Blind Test Problem 3
    - Algorithm : 재귀
    - Talk : EZ
    - ISSUE
        - 정말 아쉽다. Key = `[[1, 1, 1], [1, 1, 1], [1, 1, 1]]` Lock = `[[0, 1, 1], [1, 1, 1], [1, 1, 1]]` 이 경우가 예외 케이스이다. 27 Line, 30 Line 을 추가하여 위 경우를 타파 할 수 있었다.
...
1. BOJ 2422
    - Algorithm : Brute Force
    - Talk : 3중포문으로 해결 할 수 있다니... 무슨 알고리즘을 사용해야되는지 알고 삽질 오지게 햇다.
...
1. BOJ 9372
    ...
    - ISSUE : 참으로 황당하다..
        - 9372_2.cpp 을 보면 정말 정말 간단하다. [hojanrgid](https://www.acmicpc.net/user/hojanrgid) 의 코드를 보니까 이해가 안되는 부분이 있었는데 쓸모 없는것들 다지우고 제출하니 맞았습니다. 가 나왔다. [질문하기 풀이](https://www.acmicpc.net/board/view/38050) 여기가면 풀이를 알 수있다. 정말 이상한 문제이다.
...
## 19.1.8.
1. BOJ 13172
    - Alogirithm : Moduler ���산. 그그그 약수분해 하는 알고리즘 뭐더라 ?
