...
void go(int x, int y, int _dir, int count) {
    if(x == start_x && y == start_y) {
        if (max<count && dessert[map[x][y]]==1) max = count;
    }
    else {
        if(_dir == 4 || N<=x || N<=y || x<0 || y<0 || dessert[map[x][y]]==1) return;
        dessert[map[x][y]] =1;
        go(x+dir[_dir][0], y+dir[_dir][1], _dir, count+1);
        go(x+dir[_dir+1][0], y+dir[_dir+1][1], _dir+1, count+1);
        dessert[map[x][y]] =0;
    }
}
	...
	*/
	for (test_case = 1; test_case <= T; ++test_case)
	{
		...
		*/
        ...
        for (int i=0; i<N-1; i++) {
            for (int j=0; j<N-1; j++) {
                for(int k =0; k<101; k++) dessert[k] = 0;
                start_x = i;
                start_y = j;
                dessert[map[i][j]] =1;
                go(i+1, j+1, 0, 1);
