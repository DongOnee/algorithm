...
void turn(int chain, int dir) {
    top[chain] -= dir;
    top[chain] += 8;
    top[chain] %= 8;
}

void allturn(int start, int _dir, int end) {
    int dir = _dir;
...
void getstart(int n, int m) {
    ...
    int dir = m;
    while(0<start) {
        if(chain[start][(top[start]+6)%8] == chain[start-1][(top[start-1]+2)%8]) break;
        start--;
        dir *= -1;
    }
    while(end<3) {
        if(chain[end][(top[end]+2)%8] == chain[end+1][(top[end+1]+6)%8]) break;
        end++;
    }
    allturn(start, dir, end);
}
	...
	*/
	for (test_case = 1; test_case <= T; ++test_case)
	{
		...
		*/
        ...
        scanf("%d", &K);
        for(int i=0; i<4; i++) {
            for(int j=0; j<8; j++) scanf("%d", &chain[i][j]);
            top[i] =0;
        }
        while (K--) {
            int n, m;
        ...
        printf("#%d %d\n", test_case, chain[0][top[0]]+chain[1][top[1]]*2+chain[2][top[2]]*4+chain[3][top[3]]*8);
	}
